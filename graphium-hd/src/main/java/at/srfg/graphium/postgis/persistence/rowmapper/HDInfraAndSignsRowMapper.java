...
public class HDInfraAndSignsRowMapper implements IHDInfraAndSignsRowMapper<IHDInfraAndSigns> {

    protected static final String QUERY_PREFIX = "infra_and_signs";
    ...
    @Override
    public IHDInfraAndSigns mapRow(ResultSet rs, int i) throws SQLException {
        byte[] geometry = rs.getBytes(QUERY_PREFIX + "_geometry_ewkb");
        return new HDInfraAndSigns(rs.getLong( "id"),
                (geometry == null ? null : (Geometry) bp.parse(geometry)),
